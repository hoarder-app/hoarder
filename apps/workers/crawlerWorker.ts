...
async function getContextCookies(browser: Browser, domain?: string) {
  try {
    ...
    if (contexts.length === 0) {
      logger.info("[Crawler] No browser contexts found to extract cookies from");
      return [];
    }
  ...
  await browserMutex.runExclusive(async () => {
    try {
      ...
      if (globalBrowser) {
        ...
        logger.info(`[Crawler] Connected to browser with ${contexts.length} contexts`);
...
async function crawlPage(
  ...
  try {
    ...
    const contexts = browser.contexts();
    logger.info(`[Crawler][${jobId}] Found ${contexts.length} browser contexts`);
    ...
    logger.info(`[Crawler][${jobId}] Found ${pages.length} pages in the context`);
    ...
    if (pages.length > 0) {
      // Use the first page
      page = pages[0];
      logger.info(`[Crawler][${jobId}] Using existing page with URL: ${page.url()}`);
    } else {
