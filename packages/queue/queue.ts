...
export class SqliteQueue<T> {
  ...
  constructor(name: string, options: SqliteQueueOptions) {
    this.queueName = name;
    this.options = options;
    this.db = buildDBClient(options.path);
  }
  ...
  async enqueue(payload: T): Promise<Job> {
    const job = await this.db
      ...
      .values({
        ...
        numRunsLeft: this.options.numRetries + 1,
        maxNumRuns: this.options.numRetries + 1,
  ...
  async attemptDequeue(options: DequeueOptions): Promise<Job | null> {
      ...
      const result = await txn
        ...
        .where(
          and(
            ...
            job.allocationId == null
              ? isNull(tasksTable.allocationId)
